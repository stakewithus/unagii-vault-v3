...
contract Vault is ERC20, IERC4626, Ownership, BlockDelay {
    ...
    /// @notice performance fee on yield minted as shares to treasury
    ...
    function transfer(
        address _to,
        uint256 _amount
    ) public override useBlockDelay(msg.sender) useBlockDelay(_to) returns (bool) {
    ...
    function _report(Strategy _strategy, uint256 _harvested) internal {
        ...
        if (assets > debt) {
            ...
            uint256 net = gain + _harvested;
            fee = _calculateFee(net);
            _lockedProfit = lockedProfitBefore + net;
        } else if (debt > assets) {
            unchecked {
                loss = debt - assets;
                totalDebt -= loss;

                if (_harvested > loss) fee = _calculateFee(_harvested - loss);
                _lockedProfit = lockedProfitBefore + _harvested > loss ? lockedProfitBefore + _harvested - loss : 0;
            }
        }

        if (fee > 0) _mint(treasury, convertToShares(fee));
    ...
    function _setFee(uint16 _fee) internal {
        ...
        if (_fee > MAX_FEE) revert AboveMaximum(_fee);
    ...
    function _setTreasury(address _treasury) internal {
        if (treasury == _treasury) revert AlreadyValue();
        treasury = _treasury;
